...
const RULES_COLLECTION = "ad_rules";
const DAILY_REPORT_LIMIT = 5;
const PROMOTION_THRESHOLD = 10;
const SELECTOR_CONSISTENCY_THRESHOLD = 0.6;
const SELECTOR_LIMIT = 20;
const SELECTOR_MAX_LENGTH = 200;
const PROMOTABLE_REPORT_TYPES = [
  "gambling_site",
  "ad_selector",
];

onRecordCreateRequest((e) => {
  if (e.collection && e.collection.name !== REPORTS_COLLECTION) {
    return e.next();
  }
  ...
  const domain = normalizeDomain(e.record.get("domain"));
  const selectors = normalizeSelectors(e.record.get("selectors"));
  const reportType = String(e.record.get("report_type") || "");
  const reason = String(e.record.get("reason") || "").trim();

  validateReportPayload(domain, selectors, reportType, reason);
  enforceDailyQuota(authId);
...
onRecordAfterCreateSuccess((e) => {
  ...
  aggregateDomainReports(String(e.record.get("domain") || ""));
  return e.next();
}, REPORTS_COLLECTION);

function validateReportPayload(domain, selectors, reportType, reason) {
...
function enforceDailyQuota(authId) {
...
function aggregateDomainReports(domain) {
  if (!domain) {
    return;
  }
  ...
  const reports = candidateReports.filter((report) =>
    shouldCountForPromotion(String(report.get("report_type") || "")),
  );
  ...
  const selectorGroups = buildSelectorGroups(reports);
  ...
  if (!rule) {
    rule = new Record($app.findCollectionByNameOrId(RULES_COLLECTION));
...
function buildSelectorGroups(reports) {
  const groups = {};

  for (let i = 0; i < reports.length; i += 1) {
    const selectors = normalizeSelectors(reports[i].get("selectors"));
...
function shouldCountForPromotion(reportType) {
  return PROMOTABLE_REPORT_TYPES.includes(reportType);
}
